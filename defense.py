...
def pad_pointclouds(pointclouds, target_points=1024):
    padded = []
    for pc in pointclouds:
        if pc.shape[0] >= target_points:
            pc = pc[:target_points]  # 자르기
        else:
            pad_len = target_points - pc.shape[0]
            pad = np.repeat(pc[-1][np.newaxis, :], pad_len, axis=0)
            pc = np.concatenate([pc, pad], axis=0)
        padded.append(pc)
    return np.stack(padded, axis=0)  # shape: (N, target_points, 3)
...
def defend(data_root, one_defense, targeted=True):
    ...
    if one_defense.lower() == 'sor':
        all_defend_pc = pad_pointclouds(all_defend_pc, target_points=1024)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_root', type=str, default='data/adv_samples/uaof/pointnet2',
